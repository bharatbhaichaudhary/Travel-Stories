...
const ViewTravelStory = ({
  ...
  return (
    <div className="relative">
      <div className="flex to-current justify-end">
        <div>
        ...
        <div className="flex-1 flex flex-col gap-2 py-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs to-slate-500">
              {storyInfo && moment(storyInfo.visitedData).format("Do MMM YYYY")}
            </span>

            <div className="inline-flex items-center gap-2 text-[13px] text-cyan-600 bg-cyan-50 rounded px-2 py-1">
              <GrMapLocation className="text-sm" />
              {storyInfo &&
                storyInfo.visitedLoction.map((item, index) => {
                  storyInfo.visitedLoction.length == index + 1
                    ? `${item}`
                    : `${item}`;
                })}
            </div>
